 ...
 * ============================
 * 
 * **Purpose**: Dynamically renders form fields based on the selected material type.
 * This component provides flexibility for different resource types (books, articles, 
 * videos, etc.) to have their own specific metadata fields.
 * 
 * **Key Features**:
 * - Dynamic field rendering based on material type configuration
 * - Supports multiple field types: text, number, textarea, select, checkbox
 * - Groups fields by category for better organization
 * - Handles complex field options via JSON configuration
 * - Provides fallback displays for edge cases
 * 
 * **Field Types Supported**:
 * - `text`/`textField`: Single-line text input
 * - `number`: Numeric input with validation
 * - `textarea`: Multi-line text area
 * - `select`: Dropdown with options from options_json
 * - `checkbox`: Boolean toggle with special handling
 * 
 * **Data Flow**:
 * 1. Receives materialTypeFields array from API
 * 2. Groups fields by category for UI organization  
 * 3. Renders appropriate input type for each field
 * 4. Updates parent form's metadata object via handleFieldChange
 * 
 * **Database Integration**:
 * - Field definitions stored in material_type_fields table
 * - Field values saved to resource metadata as key-value pairs
 * - Supports dynamic field addition without code changes
 * 
 * @component
 * @example
 * <TypeSpecificFields
 *   materialTypeFields={fields}
